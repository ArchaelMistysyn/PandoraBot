...
def check_num_listings(player_obj):
    ...
    df = mydb.run_single_query(raw_query, return_value=True, params=params)
    return len(df)
...
def list_custom_item(item, cost):
    ...
    mydb.run_single_query(raw_query, params=params)
...
def get_seller_by_item(item_id):
    ...
    df = mydb.run_single_query(raw_query, return_value=True, params={'item_check': item_id})
