...
insignia_hp_list = [0, 500, 1000, 1500, 2000, 2500, 5000, 10000]
insignia_prefix = ["Dormant", "Awakened", "Evolved", "Infused", "Symbiotic", "Resonating",
                   "Mutation: Void", "Mutation: Divine"]
mutation_cost_list = ["OriginV", "OriginM"]


def display_insignia(player_object, insignia_code):
    ...
    pearl = itemrolls.augment_icons[insignia_stars - 1]
    ...
    item_rolls += f"\n{pearl} Final Damage {player_object.player_lvl * mutation_adjust:,}%"
    ...
    async def insignia_select_callback(self, interaction: discord.Interaction):
        try:
            if interaction.user.name == self.player_user.player_name:
                ...
                if self.player_user.insignia == "" or self.player_user.player_echelon < 5:
                ...
                if mutation_tier >= 2:
                    embed_msg.description = "There's nothing further I can do to improve your soul through mutation."
                    await interaction.response.edit_message(embed=embed_msg)
                    return
                ...
                token_cost = 5 if mutation_tier == 0 else 10
...
class ConfirmSelectionView(discord.ui.View):
    def __init__(self, player_user, num_selected, current_selection):
        ...
        self.num_selected = num_selected
        self.current_selection = current_selection
        self.mutation_rate = 33
        if self.current_selection == "mutation":
            if int(self.player_user.insignia[-1]) < 2:
                self.engrave.label = f"Mutate ({self.mutation_rate}%)"
            else:
                self.engrave.label = f"Mutate [MAX]"
...
def payment_embed(player_object, token_cost, current_selection):
    token_item = inventory.BasicItem("Token2")
    token_stock = inventory.check_stock(player_object, token_item.item_id)
    ...
    if type(current_selection) is list:
        for idx, x in enumerate(current_selection):
            if x != 0:
                ...
                stock = inventory.check_stock(player_object, loot_item.item_id)
